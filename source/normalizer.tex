...
We call an operation with a proper linear part the \term{first kind}, and that with an improper linear part the \term{second kind}.

\begin{screen}
  \begin{defn}[chiral crystal pattern]
...
The space group $\mathcal{G}$ does not have an operation of the second kind; otherwise, $(\bm{W}', \bm{w}')$ with $\det \bm{W}' < 0$: otherwise, $\overline{1} \circ (\bm{W}', \bm{w}')$ satisfies Eq.~\eqref{eq:chiral-crystal-patterns}.
\begin{screen}
  \begin{defn}[Sohncke group]
    A space group is called a \term{Sohncke group} if it does not contain operations of the second kind.
  \end{defn}
\end{screen}
As above, a space group of a chiral crystal pattern is a Sohncke group.

For a Sohncke group $\mathcal{G}$, we can consider a subgroup of $\mathcal{N}_{\mathcal{E}_3}(\mathcal{G})$ only with operations of the first kind,
\begin{align}
  \mathcal{N}_{\mathcal{E}_{3}}^{+}(\mathcal{G}) \coloneqq \set{ (\bm{P}, \bm{p}) \in \mathcal{N}_{\mathcal{E}_{3}}(\mathcal{G}) }{ \det \bm{P} > 0 }.
\end{align}
We call it the \term{chirality-preserving Euclidean normalizer} of $\mathcal{G}$.
\begin{screen}
  \begin{defn}[chiral space group]
    A Sohncke group $\mathcal{G}$ is called a \term{chiral space group} if its Euclidean normalizer and chirality-preserving Euclidean normalizer coincide.
  \end{defn}
\end{screen}
We can rephrase the definition; if a space group $\mathcal{G}$ and its inversion-conjugate $\overline{1} \mathcal{G} \overline{1}$ are transformed to each other by an operation of the first kind, then $\mathcal{G}$ is not a chiral space group.

In three-dimensional space-group types, there are 65 Sohncke groups, among which 11 enantiomorphic pairs are chiral space groups.
